...
public class GardienScolaireFrFRGenApiServiceImpl implements GardienScolaireFrFRGenApiService {
			...
			if(jsonObject != null) {
				...
				for(String entiteVar : entiteVars) {
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContexteFrFR.SQL_addA)
												.execute(Tuple.of(l2, "gardienCles", pk, "inscriptionCles")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
					...
					case "personneRelation":
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContexteFrFR.SQL_setD)
									.execute(Tuple.of(pk, "personneRelation", Optional.ofNullable(jsonObject.getValue(entiteVar)).map(s -> s.toString()).orElse(null))
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
						...
						futures.add(Future.future(a -> {
							...
							-> {
								if(b.succeeded())
									...
									a.handle(Future.failedFuture(new Exception("valeur GardienScolaire.photo a échoué", b.cause())));
							});
						}));
						break;
					}
								...
								executeurTravailleur.executeBlocking(
									blockingCodeHandler -> {
										try {
											...
											requeteSite.setRequeteApi_(requeteApi);
											requeteSite.getVertx().eventBus().publish("websocketGardienScolaire", JsonObject.mapFrom(requeteApi).toString());
											varsGardienScolaire(requeteSite, d -> {
												if(d.succeeded()) {
													listePUTImportGardienScolaire(requeteApi, requeteSite, e -> {
														if(e.succeeded()) {
															putimportGardienScolaireReponse(requeteSite, f -> {
																if(e.succeeded()) {
								...
								executeurTravailleur.executeBlocking(
									blockingCodeHandler -> {
										try {
											...
											varsGardienScolaire(requeteSite, d -> {
												if(d.succeeded()) {
													listePUTFusionGardienScolaire(requeteApi, requeteSite, e -> {
														if(e.succeeded()) {
															putfusionGardienScolaireReponse(requeteSite, f -> {
																if(e.succeeded()) {
										...
										try {
											rechercheGardienScolaire(requeteSite, false, true, true, "/api/gardien/copie", "PUTCopie", d -> {
												if(d.succeeded()) {
													...
													requeteSite.getVertx().eventBus().publish("websocketGardienScolaire", JsonObject.mapFrom(requeteApi).toString());
													try {
														listePUTCopieGardienScolaire(requeteApi, listeGardienScolaire, e -> {
															if(e.succeeded()) {
																putcopieGardienScolaireReponse(requeteSite, f -> {
																	if(f.succeeded()) {
	...
	public void listePUTCopieGardienScolaire(RequeteApi requeteApi, ListeRecherche<GardienScolaire> listeGardienScolaire, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		...
		listeGardienScolaire.getList().forEach(o -> {
			RequeteSiteFrFR requeteSite2 = genererRequeteSiteFrFRPourGardienScolaire(siteContexte, requeteSite.getOperationRequete(), requeteSite.getObjetJson());
				...
				for(Integer i = 0; i < entiteVars.size(); i++) {
					...
					case "inscriptionCles":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entiteVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_addA)
										.execute(Tuple.of(l, "gardienCles", pk, "inscriptionCles")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
											...
											rechercheGardienScolaire(requeteSite, false, true, true, "/api/gardien", "PATCH", d -> {
												if(d.succeeded()) {
													...
													if(listeGardienScolaire.getQueryResponse().getResults().getNumFound() > 1
														...
														try {
															listePATCHGardienScolaire(requeteApi, listeGardienScolaire, dt, e -> {
																if(e.succeeded()) {
																	patchGardienScolaireReponse(requeteSite, f -> {
																		if(f.succeeded()) {
				...
				switch(methodeNom) {
					...
					case "setPersonneRelation":
						if(jsonObject.getString(methodeNom) == null) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_removeD)
										.execute(Tuple.of(pk, "personneRelation")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
							...
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_setD)
										.execute(Tuple.of(pk, "personneRelation", o2.jsonPersonneRelation())
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
	...
	public void pagerechercheGardienScolairePageInit(GardienPage page, ListeRecherche<GardienScolaire> listeGardienScolaire) {
	}
	...
	public void reponse200PageRechercheGardienScolaire(ListeRecherche<GardienScolaire> listeGardienScolaire, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		try {
			...
			pagerechercheGardienScolairePageInit(page, listeGardienScolaire);
	...
	public void creerGardienScolaire(RequeteSiteFrFR requeteSite, Handler<AsyncResult<GardienScolaire>> gestionnaireEvenements) {
		try {
			...
			tx.preparedQuery(SiteContexteFrFR.SQL_creer)
					...
					.execute(Tuple.of(GardienScolaire.class.getCanonicalName(), utilisateurId, cree.toOffsetDateTime())
					, creerAsync
			-> {
				if(creerAsync.succeeded()) {
					...
					LOGGER.error(String.format("creerGardienScolaire a échoué. ", creerAsync.cause()));
	...
	public void rechercheGardienScolaireFq(String uri, String apiMethode, ListeRecherche<GardienScolaire> listeRecherche, String entiteVar, String valeurIndexe, String varIndexe) {
		...
		if(StringUtils.startsWith(valeurIndexe, "[")) {
			...
			String fq1 = fqs[0].equals("*") ? fqs[0] : GardienScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), fqs[0]);
			String fq2 = fqs[1].equals("*") ? fqs[1] : GardienScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), fqs[1]);
			listeRecherche.addFilterQuery(varIndexe + ":[" + fq1 + " TO " + fq2 + "]");
		} else {
			listeRecherche.addFilterQuery(varIndexe + ":" + GardienScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), valeurIndexe));
		}
	}
	...
	public void varsGardienScolaire(RequeteSiteFrFR requeteSite, Handler<AsyncResult<ListeRecherche<OperationResponse>>> gestionnaireEvenements) {
		try {
	...
	public void rechercheGardienScolaire(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode, Handler<AsyncResult<ListeRecherche<GardienScolaire>>> gestionnaireEvenements) {
		try {
			ListeRecherche<GardienScolaire> listeRecherche = rechercheGardienScolaireListe(requeteSite, peupler, stocker, modifier, uri, apiMethode);
	...
	public ListeRecherche<GardienScolaire> rechercheGardienScolaireListe(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode) {
		OperationRequest operationRequete = requeteSite.getOperationRequete();
		...
		rechercheGardienScolaire2(requeteSite, peupler, stocker, modifier, uri, apiMethode, listeRecherche);
		listeRecherche.initLoinPourClasse(requeteSite);
		return listeRecherche;
	}
	public void rechercheGardienScolaire2(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode, ListeRecherche<GardienScolaire> listeRecherche) {
	}
