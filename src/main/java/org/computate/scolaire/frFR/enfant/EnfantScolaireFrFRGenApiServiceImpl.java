...
public class EnfantScolaireFrFRGenApiServiceImpl implements EnfantScolaireFrFRGenApiService {
	...
	public void postEnfantScolaire(JsonObject body, OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		...
		requeteSite.setRequeteUri("/api/enfant");
			...
			if(jsonObject != null) {
				...
				for(String entiteVar : entiteVars) {
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContexteFrFR.SQL_addA)
												.execute(Tuple.of(l2, "enfantCle", pk, "inscriptionCles")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
					...
					case "personneDateNaissance":
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContexteFrFR.SQL_setD)
									.execute(Tuple.of(pk, "personneDateNaissance", Optional.ofNullable(jsonObject.getValue(entiteVar)).map(s -> s.toString()).orElse(null))
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
						...
						futures.add(Future.future(a -> {
							...
							-> {
								if(b.succeeded())
									...
									a.handle(Future.failedFuture(new Exception("valeur EnfantScolaire.photo a échoué", b.cause())));
							});
						}));
						break;
					}
	...
	public void putimportEnfantScolaire(JsonObject body, OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		RequeteSiteFrFR requeteSite = genererRequeteSiteFrFRPourEnfantScolaire(siteContexte, operationRequete, body);
		requeteSite.setRequeteUri("/api/enfant/import");
								...
								executeurTravailleur.executeBlocking(
									blockingCodeHandler -> {
										try {
											...
											requeteSite.setRequeteApi_(requeteApi);
											requeteSite.getVertx().eventBus().publish("websocketEnfantScolaire", JsonObject.mapFrom(requeteApi).toString());
											varsEnfantScolaire(requeteSite, d -> {
												if(d.succeeded()) {
	...
	public void putfusionEnfantScolaire(JsonObject body, OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		RequeteSiteFrFR requeteSite = genererRequeteSiteFrFRPourEnfantScolaire(siteContexte, operationRequete, body);
		requeteSite.setRequeteUri("/api/enfant/fusion");
	...
	public void putcopieEnfantScolaire(JsonObject body, OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		RequeteSiteFrFR requeteSite = genererRequeteSiteFrFRPourEnfantScolaire(siteContexte, operationRequete, body);
		requeteSite.setRequeteUri("/api/enfant/copie");
										...
										try {
											rechercheEnfantScolaire(requeteSite, false, true, true, "/api/enfant/copie", "PUTCopie", d -> {
												if(d.succeeded()) {
													...
													requeteSite.getVertx().eventBus().publish("websocketEnfantScolaire", JsonObject.mapFrom(requeteApi).toString());
													try {
														listePUTCopieEnfantScolaire(requeteApi, listeEnfantScolaire, e -> {
															if(e.succeeded()) {
																putcopieEnfantScolaireReponse(requeteSite, f -> {
																	if(f.succeeded()) {
	...
	public void listePUTCopieEnfantScolaire(RequeteApi requeteApi, ListeRecherche<EnfantScolaire> listeEnfantScolaire, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		...
		listeEnfantScolaire.getList().forEach(o -> {
			RequeteSiteFrFR requeteSite2 = genererRequeteSiteFrFRPourEnfantScolaire(siteContexte, requeteSite.getOperationRequete(), requeteSite.getObjetJson());
			requeteSite2.setRequeteApi_(requeteSite.getRequeteApi_());
			o.setRequeteSite_(requeteSite2);
			futures.add(
				putcopieEnfantScolaireFuture(requeteSite2, JsonObject.mapFrom(o), a -> {
					if(a.succeeded()) {
					} else {
				...
				for(Integer i = 0; i < entiteVars.size(); i++) {
					...
					case "inscriptionCles":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entiteVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_addA)
										.execute(Tuple.of(l, "enfantCle", pk, "inscriptionCles")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
											...
											rechercheEnfantScolaire(requeteSite, false, true, true, "/api/enfant", "PATCH", d -> {
												if(d.succeeded()) {
													...
													if(listeEnfantScolaire.getQueryResponse().getResults().getNumFound() > 1
														...
														try {
															listePATCHEnfantScolaire(requeteApi, listeEnfantScolaire, dt, e -> {
																if(e.succeeded()) {
																	patchEnfantScolaireReponse(requeteSite, f -> {
																		if(f.succeeded()) {
				...
				switch(methodeNom) {
					...
					case "setPersonneDateNaissance":
						if(jsonObject.getString(methodeNom) == null) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_removeD)
										.execute(Tuple.of(pk, "personneDateNaissance")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
							...
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_setD)
										.execute(Tuple.of(pk, "personneDateNaissance", o2.jsonPersonneDateNaissance())
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
	...
	public void getEnfantScolaire(OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		...
		requeteSite.setRequeteUri("/api/enfant/{id}");
	...
	public void rechercheadminEnfantScolaire(OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		RequeteSiteFrFR requeteSite = genererRequeteSiteFrFRPourEnfantScolaire(siteContexte, operationRequete);
		requeteSite.setRequeteUri("/api/admin/enfant");
	...
	public void pagerechercheEnfantScolaire(OperationRequest operationRequete, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		RequeteSiteFrFR requeteSite = genererRequeteSiteFrFRPourEnfantScolaire(siteContexte, operationRequete);
		requeteSite.setRequeteUri("/enfant");
	...
	public void pagerechercheEnfantScolairePageInit(EnfantPage page, ListeRecherche<EnfantScolaire> listeEnfantScolaire) {
	}
	...
	public void reponse200PageRechercheEnfantScolaire(ListeRecherche<EnfantScolaire> listeEnfantScolaire, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
		try {
			...
			pagerechercheEnfantScolairePageInit(page, listeEnfantScolaire);
	...
	public void creerEnfantScolaire(RequeteSiteFrFR requeteSite, Handler<AsyncResult<EnfantScolaire>> gestionnaireEvenements) {
		try {
			...
			tx.preparedQuery(SiteContexteFrFR.SQL_creer)
					...
					.execute(Tuple.of(EnfantScolaire.class.getCanonicalName(), utilisateurId, cree.toOffsetDateTime())
					, creerAsync
			-> {
	...
	public void rechercheEnfantScolaireFq(String uri, String apiMethode, ListeRecherche<EnfantScolaire> listeRecherche, String entiteVar, String valeurIndexe, String varIndexe) {
		...
		if(StringUtils.startsWith(valeurIndexe, "[")) {
			...
			String fq1 = fqs[0].equals("*") ? fqs[0] : EnfantScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), fqs[0]);
			String fq2 = fqs[1].equals("*") ? fqs[1] : EnfantScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), fqs[1]);
			listeRecherche.addFilterQuery(varIndexe + ":[" + fq1 + " TO " + fq2 + "]");
		} else {
			listeRecherche.addFilterQuery(varIndexe + ":" + EnfantScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), valeurIndexe));
		}
	}
	...
	public void varsEnfantScolaire(RequeteSiteFrFR requeteSite, Handler<AsyncResult<ListeRecherche<OperationResponse>>> gestionnaireEvenements) {
		try {
	...
	public void rechercheEnfantScolaire(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode, Handler<AsyncResult<ListeRecherche<EnfantScolaire>>> gestionnaireEvenements) {
		try {
			ListeRecherche<EnfantScolaire> listeRecherche = rechercheEnfantScolaireListe(requeteSite, peupler, stocker, modifier, uri, apiMethode);
	...
	public ListeRecherche<EnfantScolaire> rechercheEnfantScolaireListe(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode) {
		OperationRequest operationRequete = requeteSite.getOperationRequete();
		...
		rechercheEnfantScolaire2(requeteSite, peupler, stocker, modifier, uri, apiMethode, listeRecherche);
		listeRecherche.initLoinPourClasse(requeteSite);
		return listeRecherche;
	}
	public void rechercheEnfantScolaire2(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode, ListeRecherche<EnfantScolaire> listeRecherche) {
	}
