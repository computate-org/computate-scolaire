...
public class MereScolaireFrFRGenApiServiceImpl implements MereScolaireFrFRGenApiService {
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContexteFrFR.SQL_addA)
												.execute(Tuple.of(pk, "inscriptionCles", l2, "mereCles")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
				...
				for(Integer i = 0; i < entiteVars.size(); i++) {
					...
					case "inscriptionCles":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entiteVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContexteFrFR.SQL_addA)
										.execute(Tuple.of(pk, "inscriptionCles", l, "mereCles")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
											...
											rechercheMereScolaire(requeteSite, false, true, true, "/api/mere", "PATCH", d -> {
												if(d.succeeded()) {
													...
													if(listeMereScolaire.getQueryResponse().getResults().getNumFound() > 1
														...
														erreurMereScolaire(requeteSite, gestionnaireEvenements, Future.failedFuture(message));
													} else {
	...
	public void creerMereScolaire(RequeteSiteFrFR requeteSite, Handler<AsyncResult<MereScolaire>> gestionnaireEvenements) {
		try {
			...
			tx.preparedQuery(SiteContexteFrFR.SQL_creer)
					...
					.execute(Tuple.of(MereScolaire.class.getCanonicalName(), utilisateurId, cree.toOffsetDateTime())
					, creerAsync
			-> {
	...
	public void rechercheMereScolaireFq(String uri, String apiMethode, ListeRecherche<MereScolaire> listeRecherche, String entiteVar, String valeurIndexe, String varIndexe) {
		...
		if(StringUtils.startsWith(valeurIndexe, "[")) {
			...
			String fq1 = fqs[0].equals("*") ? fqs[0] : MereScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), fqs[0]);
			String fq2 = fqs[1].equals("*") ? fqs[1] : MereScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), fqs[1]);
			listeRecherche.addFilterQuery(varIndexe + ":[" + fq1 + " TO " + fq2 + "]");
		} else {
			listeRecherche.addFilterQuery(varIndexe + ":" + MereScolaire.staticSolrFqPourClasse(entiteVar, listeRecherche.getRequeteSite_(), valeurIndexe));
		}
	}
	...
	public void rechercheMereScolaire(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode, Handler<AsyncResult<ListeRecherche<MereScolaire>>> gestionnaireEvenements) {
		try {
			ListeRecherche<MereScolaire> listeRecherche = rechercheMereScolaireListe(requeteSite, peupler, stocker, modifier, uri, apiMethode);
	...
	public ListeRecherche<MereScolaire> rechercheMereScolaireListe(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode) {
		OperationRequest operationRequete = requeteSite.getOperationRequete();
		...
		rechercheMereScolaire2(requeteSite, peupler, stocker, modifier, uri, apiMethode, listeRecherche);
		listeRecherche.initLoinPourClasse(requeteSite);
		return listeRecherche;
	}
	public void rechercheMereScolaire2(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, Boolean modifier, String uri, String apiMethode, ListeRecherche<MereScolaire> listeRecherche) {
	}
