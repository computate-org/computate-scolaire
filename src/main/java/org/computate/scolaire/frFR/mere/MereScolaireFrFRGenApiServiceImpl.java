...
public class MereScolaireFrFRGenApiServiceImpl implements MereScolaireFrFRGenApiService {
						...
						if(b.succeeded()) {
							rechercheMereScolaire(requeteSite, false, true, null, c -> {
								if(c.succeeded()) {
									...
									listePATCHMereScolaire(listeMereScolaire, d -> {
										if(d.succeeded()) {
											SQLConnection connexionSql = requeteSite.getConnexionSql();
	...
	public void listePATCHMereScolaire(ListeRecherche<MereScolaire> listeMereScolaire, Handler<AsyncResult<OperationResponse>> gestionnaireEvenements) {
								...
								if(c.succeeded()) {
									indexerMereScolaire(mereScolaire, d -> {
										if(d.succeeded()) {
											...
											erreurMereScolaire(o.getRequeteSite_(), gestionnaireEvenements, d);
										}
									});
								} else {
									erreurMereScolaire(o.getRequeteSite_(), gestionnaireEvenements, c);
								}
							});
						} else {
							erreurMereScolaire(o.getRequeteSite_(), gestionnaireEvenements, b);
						}
					});
				} else {
					erreurMereScolaire(o.getRequeteSite_(), gestionnaireEvenements, a);
				}
			});
			return future;
		} catch(Exception e) {
