		...
		try {
			...
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
					...
					LOGGER.error(String.format("postCluster a échoué. ", a.cause()));
		...
		try {
			...
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
							...
							putimportClusterReponse(requeteSite, c -> {
								if(c.succeeded()) {
									...
									executeurTravailleur.executeBlocking(
										blockingCodeHandler -> {
											RequeteSiteFrFR requeteSite2 = genererRequeteSiteFrFRPourCluster(siteContexte, operationRequete, body);
											try {
												RequeteApi requeteApi = new RequeteApi();
												...
												requeteSite2.getVertx().eventBus().publish("websocketCluster", JsonObject.mapFrom(requeteApi).toString());
												sqlCluster(requeteSite2, d -> {
													if(d.succeeded()) {
														listePUTImportCluster(requeteApi, requeteSite2, e -> {
															if(e.succeeded()) {
																putimportClusterReponse(requeteSite2, f -> {
																	if(f.succeeded()) {
					...
					LOGGER.error(String.format("putimportCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			...
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
									...
									executeurTravailleur.executeBlocking(
										blockingCodeHandler -> {
											...
											try {
												...
												sqlCluster(requeteSite2, d -> {
													if(d.succeeded()) {
														listePUTFusionCluster(requeteApi, requeteSite2, e -> {
															if(e.succeeded()) {
																putfusionClusterReponse(requeteSite2, f -> {
																	if(f.succeeded()) {
					...
					LOGGER.error(String.format("putfusionCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			...
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
								...
								if(c.succeeded()) {
									...
									executeurTravailleur.executeBlocking(
										blockingCodeHandler -> {
											RequeteSiteFrFR requeteSite2 = genererRequeteSiteFrFRPourCluster(siteContexte, operationRequete, body);
											try {
												rechercheCluster(requeteSite2, false, true, "/api/cluster/copie", "PUTCopie", d -> {
													if(d.succeeded()) {
					...
					LOGGER.error(String.format("putcopieCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			...
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
								...
								if(c.succeeded()) {
									...
									executeurTravailleur.executeBlocking(
										blockingCodeHandler -> {
											RequeteSiteFrFR requeteSite2 = genererRequeteSiteFrFRPourCluster(siteContexte, operationRequete, body);
											try {
												rechercheCluster(requeteSite2, false, true, "/api/cluster", "PATCH", d -> {
													if(d.succeeded()) {
														ListeRecherche<Cluster> listeCluster = d.result();
														RequeteApi requeteApi = new RequeteApi();
														requeteApi.setRows(listeCluster.getRows());
															...
															if(e.succeeded()) {
																try {
																	listePATCHCluster(requeteApi, listeCluster, dt, f -> {
																		if(f.succeeded()) {
																			patchClusterReponse(requeteSite2, g -> {
																				if(g.succeeded()) {
					...
					LOGGER.error(String.format("patchCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
					...
					LOGGER.error(String.format("getCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
					...
					LOGGER.error(String.format("rechercheCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
		...
		try {
			sqlCluster(requeteSite, a -> {
				if(a.succeeded()) {
					...
					LOGGER.error(String.format("pagerechercheCluster a échoué. ", a.cause()));
					erreurCluster(requeteSite, gestionnaireEvenements, a);
				}
			});
		} catch(Exception ex) {
