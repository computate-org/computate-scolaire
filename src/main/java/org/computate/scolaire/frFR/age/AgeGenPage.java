...
public class AgeGenPage extends AgeGenPageGen<ClusterPage> {
	...
	@Override public void htmlScriptAgeGenPage() {
		...
		if(
			...
			tl(2, "suggereAgeScolaireAnneeCle([{'name':'fq','value':'ageCles:' + pk}], $('#listAgeScolaireAnneeCle_Page'), pk, true); ");
		} else {
			tl(2, "suggereAgeScolaireAnneeCle([{'name':'fq','value':'ageCles:' + pk}], $('#listAgeScolaireAnneeCle_Page'), pk, false); ");
		}
		if(
				CollectionUtils.containsAny(requeteSite_.getUtilisateurRolesRessource(), ROLES)
				|| CollectionUtils.containsAny(requeteSite_.getUtilisateurRolesRoyaume(), ROLES)
		...
		try {
			...
			{ p.e("div").a("class", "w3-bar ").f();
				...
				p.e("input")
					...
					.a("autocomplete", "off")
					.a("oninput", "suggereAgeScolaireObjetSuggere( [ { 'name': 'q', 'value': 'objetSuggere:' + $(this).val() }, { 'name': 'rows', 'value': '10' }, { 'name': 'fl', 'value': 'pk,pageUrlPk,ageNomComplet' } ], $('#suggereListAgeScolaire", id, "'), ", p.getRequeteSite_().getRequetePk(), "); ")
