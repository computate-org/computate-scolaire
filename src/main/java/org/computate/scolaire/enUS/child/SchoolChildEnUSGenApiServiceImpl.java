...
public class SchoolChildEnUSGenApiServiceImpl implements SchoolChildEnUSGenApiService {
					...
					if(selectCAsync.succeeded()) {
						try {
							...
							if(userValues == null) {
								...
								userSchoolChildDefine(siteRequest, jsonObject);
								...
								jsonObject.put("userLastName", jsonPrincipal.getString("family_name"));
								jsonObject.put("userId", jsonPrincipal.getString("sub"));
								Boolean define = userSchoolChildDefine(siteRequest, jsonObject);
								if(define) {
									SiteUser siteUser;
									if(siteUser1 == null) {
	...
	public Boolean userSchoolChildDefine(SiteRequestEnUS siteRequest, JsonObject jsonObject) {
		return true;
	}
