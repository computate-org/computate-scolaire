...
public class SchoolChildEnUSGenApiServiceImpl implements SchoolChildEnUSGenApiService {
			...
			if(jsonObject != null) {
				...
				for(String entityVar : entityVars) {
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContextEnUS.SQL_addA)
												.execute(Tuple.of(l2, "childKey", pk, "enrollmentKeys")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
					...
					case "personBirthDate":
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContextEnUS.SQL_setD)
									.execute(Tuple.of(pk, "personBirthDate", Optional.ofNullable(jsonObject.getValue(entityVar)).map(s -> s.toString()).orElse(null))
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
						...
						putcopySchoolChildResponse(siteRequest, c -> {
							if(c.succeeded()) {
								...
								workerExecutor.executeBlocking(
									blockingCodeHandler -> {
										try {
											aSearchSchoolChild(siteRequest, false, true, true, "/api/child/copy", "PUTCopy", d -> {
												if(d.succeeded()) {
				...
				for(Integer i = 0; i < entityVars.size(); i++) {
					...
					case "enrollmentKeys":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entityVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_addA)
										.execute(Tuple.of(l, "childKey", pk, "enrollmentKeys")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
						...
						patchSchoolChildResponse(siteRequest, c -> {
							if(c.succeeded()) {
								...
								workerExecutor.executeBlocking(
									blockingCodeHandler -> {
										try {
											aSearchSchoolChild(siteRequest, false, true, true, "/api/child", "PATCH", d -> {
												if(d.succeeded()) {
													SearchList<SchoolChild> listSchoolChild = d.result();

													List<String> roles2 = Arrays.asList("SiteAdmin");
													if(listSchoolChild.getQueryResponse().getResults().getNumFound() > 1
															&& !CollectionUtils.containsAny(siteRequest.getUserResourceRoles(), roles2)
															&& !CollectionUtils.containsAny(siteRequest.getUserRealmRoles(), roles2)
															) {
														String message = String.format("roles required: " + String.join(", ", roles2));
														LOGGER.error(message);
														errorSchoolChild(siteRequest, eventHandler, Future.failedFuture(message));
													} else {
				...
				switch(methodName) {
					...
					case "setPersonBirthDate":
						if(jsonObject.getString(methodName) == null) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_removeD)
										.execute(Tuple.of(pk, "personBirthDate")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
							...
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_setD)
										.execute(Tuple.of(pk, "personBirthDate", o2.jsonPersonBirthDate())
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
		...
		try {
			{
				userSchoolChild(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSchoolChild(siteRequest, false, true, false, "/api/child/{id}", "GET", c -> {
							if(c.succeeded()) {
		...
		try {
			{
				userSchoolChild(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSchoolChild(siteRequest, false, true, false, "/api/child", "Search", c -> {
							if(c.succeeded()) {
								SearchList<SchoolChild> listSchoolChild = c.result();
		...
		try {
			{
				userSchoolChild(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSchoolChild(siteRequest, false, true, false, "/api/admin/child", "AdminSearch", c -> {
							if(c.succeeded()) {
								SearchList<SchoolChild> listSchoolChild = c.result();
		...
		try {
			{
				userSchoolChild(siteRequest, b -> {
					if(b.succeeded()) {
						aSearchSchoolChild(siteRequest, false, true, false, "/child", "SearchPage", c -> {
							if(c.succeeded()) {
								SearchList<SchoolChild> listSchoolChild = c.result();
	...
	public void createSchoolChild(SiteRequestEnUS siteRequest, Handler<AsyncResult<SchoolChild>> eventHandler) {
		try {
			...
			tx.preparedQuery(SiteContextEnUS.SQL_create)
					...
					.execute(Tuple.of(SchoolChild.class.getCanonicalName(), userId, created.toOffsetDateTime())
					, createAsync
			-> {
	...
	public void aSearchSchoolChild(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, Handler<AsyncResult<SearchList<SchoolChild>>> eventHandler) {
		try {
			SearchList<SchoolChild> searchList = aSearchSchoolChildList(siteRequest, populate, store, modify, uri, apiMethod);
	...
	public SearchList<SchoolChild> aSearchSchoolChildList(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod) {
		OperationRequest operationRequest = siteRequest.getOperationRequest();
		...
		aSearchSchoolChild2(siteRequest, populate, store, modify, uri, apiMethod, searchList);
		searchList.initDeepForClass(siteRequest);
		return searchList;
	}
	public void aSearchSchoolChild2(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, SearchList<SchoolChild> searchList) {
	}
