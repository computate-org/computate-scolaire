...
public class SiteContextEnUS extends SiteContextEnUSGen<Object> {
	...
	public static final String SQL_exists = "select count(*), nom_canonique, id_utilisateur from c group by nom_canonique, id_utilisateur having c.pk=?;\n";

	public static final String SQL_create = "insert into c(nom_canonique, id_utilisateur) values(?, ?) returning pk;\n";

	public static final String SQL_modify = "update c set modifie=now() where pk=? and nom_canonique=? returning cree;\n";
	...
	public static final String SQL_define = "select chemin, valeur from d where d.pk_c=? and d.actuel union select 'cree', to_char(cree, 'YYYY-MM-DD\"T\"HH24:MI:SS.USOF\":00\"') from c where pk=? union select 'modifie', to_char(modifie, 'YYYY-MM-DD\"T\"HH24:MI:SS.USOF\":00\"') from c where pk=?;\n";

	public static final String SQL_attribute = "select pk1, pk2, entite1, entite2 from a where (a.pk1=? or a.pk2=?) and a.actuel=true;\n";
