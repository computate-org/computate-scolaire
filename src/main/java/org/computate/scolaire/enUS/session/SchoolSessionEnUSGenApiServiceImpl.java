...
public class SchoolSessionEnUSGenApiServiceImpl implements SchoolSessionEnUSGenApiService {
	...
	public void postSchoolSession(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		siteRequest.setRequestUri("/api/session");
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContextEnUS.SQL_addA)
												.execute(Tuple.of(pk, "ageKeys", l2, "sessionKey")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
					...
					case "seasonKey":
						{
							...
							if(l != null) {
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContextEnUS.SQL_addA)
												.execute(Tuple.of(pk, "seasonKey", l2, "sessionKeys")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
	...
	public void putimportSchoolSession(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolSession(siteContext, operationRequest, body);
		siteRequest.setRequestUri("/api/session/import");
								...
								workerExecutor.executeBlocking(
									blockingCodeHandler -> {
										try {
											...
											siteRequest.setApiRequest_(apiRequest);
											siteRequest.getVertx().eventBus().publish("websocketSchoolSession", JsonObject.mapFrom(apiRequest).toString());
											varsSchoolSession(siteRequest, d -> {
												if(d.succeeded()) {
	...
	public void putmergeSchoolSession(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolSession(siteContext, operationRequest, body);
		siteRequest.setRequestUri("/api/session/merge");
	...
	public void putcopySchoolSession(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolSession(siteContext, operationRequest, body);
		siteRequest.setRequestUri("/api/session/copy");
										...
										try {
											aSearchSchoolSession(siteRequest, false, true, true, "/api/session/copy", "PUTCopy", d -> {
												if(d.succeeded()) {
													...
													siteRequest.getVertx().eventBus().publish("websocketSchoolSession", JsonObject.mapFrom(apiRequest).toString());
													try {
														listPUTCopySchoolSession(apiRequest, listSchoolSession, e -> {
															if(e.succeeded()) {
																putcopySchoolSessionResponse(siteRequest, f -> {
																	if(f.succeeded()) {
	...
	public void listPUTCopySchoolSession(ApiRequest apiRequest, SearchList<SchoolSession> listSchoolSession, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		listSchoolSession.getList().forEach(o -> {
			SiteRequestEnUS siteRequest2 = generateSiteRequestEnUSForSchoolSession(siteContext, siteRequest.getOperationRequest(), siteRequest.getJsonObject());
				...
				for(Integer i = 0; i < entityVars.size(); i++) {
					...
					case "ageKeys":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entityVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_addA)
										.execute(Tuple.of(pk, "ageKeys", l, "sessionKey")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
					...
					case "seasonKey":
						...
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContextEnUS.SQL_addA)
									.execute(Tuple.of(pk, "seasonKey", l, "sessionKeys")
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
											...
											aSearchSchoolSession(siteRequest, false, true, true, "/api/session", "PATCH", d -> {
												if(d.succeeded()) {
													...
													if(listSchoolSession.getQueryResponse().getResults().getNumFound() > 1
														...
														errorSchoolSession(siteRequest, eventHandler, Future.failedFuture(message));
													} else {
	...
	public void getSchoolSession(OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		siteRequest.setRequestUri("/api/session/{id}");
	...
	public void searchpageSchoolSession(OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolSession(siteContext, operationRequest);
		siteRequest.setRequestUri("/session");
	...
	public void createSchoolSession(SiteRequestEnUS siteRequest, Handler<AsyncResult<SchoolSession>> eventHandler) {
		try {
			...
			tx.preparedQuery(SiteContextEnUS.SQL_create)
					...
					.execute(Tuple.of(SchoolSession.class.getCanonicalName(), userId, created.toOffsetDateTime())
					, createAsync
			-> {
				if(createAsync.succeeded()) {
					...
					LOGGER.error(String.format("createSchoolSession failed. ", createAsync.cause()));
	...
	public void aSearchSchoolSessionFq(String uri, String apiMethod, SearchList<SchoolSession> searchList, String entityVar, String valueIndexed, String varIndexed) {
		...
		if(StringUtils.startsWith(valueIndexed, "[")) {
			...
			String fq1 = fqs[0].equals("*") ? fqs[0] : SchoolSession.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), fqs[0]);
			String fq2 = fqs[1].equals("*") ? fqs[1] : SchoolSession.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), fqs[1]);
			searchList.addFilterQuery(varIndexed + ":[" + fq1 + " TO " + fq2 + "]");
		} else {
			searchList.addFilterQuery(varIndexed + ":" + SchoolSession.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), valueIndexed));
		}
	}
	...
	public void varsSchoolSession(SiteRequestEnUS siteRequest, Handler<AsyncResult<SearchList<OperationResponse>>> eventHandler) {
		try {
	...
	public void aSearchSchoolSession(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, Handler<AsyncResult<SearchList<SchoolSession>>> eventHandler) {
		try {
			SearchList<SchoolSession> searchList = aSearchSchoolSessionList(siteRequest, populate, store, modify, uri, apiMethod);
	...
	public SearchList<SchoolSession> aSearchSchoolSessionList(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod) {
		OperationRequest operationRequest = siteRequest.getOperationRequest();
		...
		aSearchSchoolSession2(siteRequest, populate, store, modify, uri, apiMethod, searchList);
		searchList.initDeepForClass(siteRequest);
		return searchList;
	}
	public void aSearchSchoolSession2(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, SearchList<SchoolSession> searchList) {
	}
