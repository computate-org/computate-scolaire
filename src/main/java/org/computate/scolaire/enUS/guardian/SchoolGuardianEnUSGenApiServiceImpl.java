...
public class SchoolGuardianEnUSGenApiServiceImpl implements SchoolGuardianEnUSGenApiService {
						...
						if(b.succeeded()) {
							aSearchSchoolGuardian(siteRequest, false, true, null, c -> {
								if(c.succeeded()) {
									...
									listPATCHSchoolGuardian(listSchoolGuardian, d -> {
										if(d.succeeded()) {
											SQLConnection sqlConnection = siteRequest.getSqlConnection();
	...
	public void listPATCHSchoolGuardian(SearchList<SchoolGuardian> listSchoolGuardian, Handler<AsyncResult<OperationResponse>> eventHandler) {
								...
								if(c.succeeded()) {
									indexSchoolGuardian(schoolGuardian, d -> {
										if(d.succeeded()) {
											...
											errorSchoolGuardian(o.getSiteRequest_(), eventHandler, d);
										}
									});
								} else {
									errorSchoolGuardian(o.getSiteRequest_(), eventHandler, c);
								}
							});
						} else {
							errorSchoolGuardian(o.getSiteRequest_(), eventHandler, b);
						}
					});
				} else {
					errorSchoolGuardian(o.getSiteRequest_(), eventHandler, a);
				}
			});
			return future;
		} catch(Exception e) {
	...
	public String varIndexedSchoolGuardian(String entityVar) {
		switch(entityVar) {
			...
			case "guardianId":
				return "guardianId_indexed_string";
			case "pageUrl":
	...
	public void aSearchSchoolGuardian(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, String classApiUriMethod, Handler<AsyncResult<SearchList<SchoolGuardian>>> eventHandler) {
		try {
			...
			listSearch.addFilterQuery("classCanonicalNames_indexed_strings:" + ClientUtils.escapeQueryChars("org.computate.scolaire.enUS.guardian.SchoolGuardian"));
			...
			if(id != null) {
				listSearch.addFilterQuery("(id:" + ClientUtils.escapeQueryChars(id) + " OR guardianId_indexed_string:" + ClientUtils.escapeQueryChars(id) + ")");
			}
