			...
			if(jsonObject != null) {
				...
				for(String entityVar : entityVars) {
					...
					case "htmlIfVarEquals":
						futures.add(Future.future(a -> {
							tx.preparedQuery(SiteContextEnUS.SQL_setD)
									.execute(Tuple.of(pk, "htmlIfVarEquals", Optional.ofNullable(jsonObject.getValue(entityVar)).map(s -> s.toString()).orElse(null))
									, b
							-> {
								if(b.succeeded())
									a.handle(Future.succeededFuture());
								else
									a.handle(Future.failedFuture(new Exception("value HtmlPart.htmlIfVarEquals failed", b.cause())));
							});
						}));
						break;
					case "htmlVarForEach":
				...
				switch(methodName) {
					...
					case "setHtmlIfVarEquals":
						if(jsonObject.getString(methodName) == null) {
							...
							o2.setHtmlIfVarEquals(jsonObject.getString(methodName));
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_setD)
