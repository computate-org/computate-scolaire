...
public class SchoolMomEnUSGenApiServiceImpl implements SchoolMomEnUSGenApiService {
						...
						if(b.succeeded()) {
							aSearchSchoolMom(siteRequest, false, true, null, c -> {
								if(c.succeeded()) {
									...
									listPATCHSchoolMom(listSchoolMom, d -> {
										if(d.succeeded()) {
											SQLConnection sqlConnection = siteRequest.getSqlConnection();
	...
	public void listPATCHSchoolMom(SearchList<SchoolMom> listSchoolMom, Handler<AsyncResult<OperationResponse>> eventHandler) {
								...
								if(c.succeeded()) {
									indexSchoolMom(schoolMom, d -> {
										if(d.succeeded()) {
											...
											errorSchoolMom(o.getSiteRequest_(), eventHandler, d);
										}
									});
								} else {
									errorSchoolMom(o.getSiteRequest_(), eventHandler, c);
								}
							});
						} else {
							errorSchoolMom(o.getSiteRequest_(), eventHandler, b);
						}
					});
				} else {
					errorSchoolMom(o.getSiteRequest_(), eventHandler, a);
				}
			});
			return future;
		} catch(Exception e) {
