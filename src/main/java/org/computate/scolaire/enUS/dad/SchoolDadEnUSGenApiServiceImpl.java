...
public class SchoolDadEnUSGenApiServiceImpl implements SchoolDadEnUSGenApiService {
											...
											workerExecutor.executeBlocking(
												blockingCodeHandler -> {
													sqlSchoolDad(siteRequest, e -> {
														if(e.succeeded()) {
															try {
																listPUTImportSchoolDad(apiRequest, listSchoolDad, f -> {
																	if(f.succeeded()) {
																		putimportSchoolDadResponse(listSchoolDad, g -> {
																			if(g.succeeded()) {
																				eventHandler.handle(Future.succeededFuture(g.result()));
																				LOGGER.info(String.format("putimportSchoolDad succeeded. "));
																			} else {
																				LOGGER.error(String.format("putimportSchoolDad failed. ", g.cause()));
																				errorSchoolDad(siteRequest, eventHandler, d);
																			}
																		});
																	} else {
	...
	public void listPUTImportSchoolDad(ApiRequest apiRequest, SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		jsonArray.forEach(o -> {
			...
			futures.add(
				putimportSchoolDadFuture(siteRequest, jsonObject, a -> {
					if(a.succeeded()) {
						SchoolDad schoolDad = a.result();
						apiRequestSchoolDad(schoolDad);
					} else {
		...
		CompositeFuture.all(futures).setHandler( a -> {
			if(a.succeeded()) {
				...
				response200PUTImportSchoolDad(listSchoolDad, eventHandler);
			} else {
	...
	public Future<SchoolDad> putimportSchoolDadFuture(SiteRequestEnUS siteRequest, JsonObject jsonObject, Handler<AsyncResult<SchoolDad>> eventHandler) {
		Promise<SchoolDad> promise = Promise.promise();
		try {
			...
			createSchoolDad(siteRequest, a -> {
				if(a.succeeded()) {
					SchoolDad schoolDad = a.result();
					sqlPUTImportSchoolDad(schoolDad, jsonObject, b -> {
						if(b.succeeded()) {
	...
	public void sqlPUTImportSchoolDad(SchoolDad o, JsonObject jsonObject, Handler<AsyncResult<OperationResponse>> eventHandler) {
		try {
			SiteRequestEnUS siteRequest = o.getSiteRequest_();
			SQLConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
					...
					case "enrollmentKeys":
						for(Long l : jsonObject.getJsonArray(entityVar).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							...
							if(searchList.size() == 1) {
								...
								putSqlParams.addAll(Arrays.asList("dadKeys", searchList.get(0).getPk(), "enrollmentKeys", pk));
							}
						}
						break;
					case "personFirstName":
	...
	public void putimportSchoolDadResponse(SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
		response200PUTImportSchoolDad(listSchoolDad, a -> {
			if(a.succeeded()) {
				SQLConnection sqlConnection = siteRequest.getSqlConnection();
				sqlConnection.commit(b -> {
					if(b.succeeded()) {
	...
	public void response200PUTImportSchoolDad(SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		try {
			SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
											...
											workerExecutor.executeBlocking(
												blockingCodeHandler -> {
													sqlSchoolDad(siteRequest, e -> {
														if(e.succeeded()) {
															try {
																listPUTMergeSchoolDad(apiRequest, listSchoolDad, f -> {
																	if(f.succeeded()) {
																		putmergeSchoolDadResponse(listSchoolDad, g -> {
																			if(g.succeeded()) {
																				eventHandler.handle(Future.succeededFuture(g.result()));
																				LOGGER.info(String.format("putmergeSchoolDad succeeded. "));
																			} else {
																				LOGGER.error(String.format("putmergeSchoolDad failed. ", g.cause()));
																				errorSchoolDad(siteRequest, eventHandler, d);
																			}
																		});
																	} else {
	...
	public void listPUTMergeSchoolDad(ApiRequest apiRequest, SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		List<Future> futures = new ArrayList<>();
		SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
		JsonArray jsonArray = Optional.ofNullable(siteRequest.getJsonObject()).map(o -> o.getJsonArray("list")).orElse(new JsonArray());
		jsonArray.forEach(o -> {
			JsonObject jsonObject = (JsonObject)o;
			futures.add(
				putmergeSchoolDadFuture(siteRequest, jsonObject, a -> {
					if(a.succeeded()) {
						SchoolDad schoolDad = a.result();
						apiRequestSchoolDad(schoolDad);
					} else {
		...
		CompositeFuture.all(futures).setHandler( a -> {
			if(a.succeeded()) {
				apiRequest.setNumPATCH(apiRequest.getNumPATCH() + jsonArray.size());
				response200PUTMergeSchoolDad(listSchoolDad, eventHandler);
			} else {
				errorSchoolDad(apiRequest.getSiteRequest_(), eventHandler, a);
			}
		});
	...
	public Future<SchoolDad> putmergeSchoolDadFuture(SiteRequestEnUS siteRequest, JsonObject jsonObject, Handler<AsyncResult<SchoolDad>> eventHandler) {
		Promise<SchoolDad> promise = Promise.promise();
		try {
			...
			createSchoolDad(siteRequest, a -> {
				if(a.succeeded()) {
					SchoolDad schoolDad = a.result();
					sqlPUTMergeSchoolDad(schoolDad, jsonObject, b -> {
						if(b.succeeded()) {
							defineSchoolDad(schoolDad, c -> {
								if(c.succeeded()) {
									attributeSchoolDad(schoolDad, d -> {
	...
	public void sqlPUTMergeSchoolDad(SchoolDad o, JsonObject jsonObject, Handler<AsyncResult<OperationResponse>> eventHandler) {
		try {
			SiteRequestEnUS siteRequest = o.getSiteRequest_();
			SQLConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			...
			if(jsonObject != null) {
				...
				for(Integer i = 0; i < entityVars.size(); i++) {
					...
					case "enrollmentKeys":
						for(Long l : jsonObject.getJsonArray(entityVar).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							putSql.append(SiteContextEnUS.SQL_addA);
							putSqlParams.addAll(Arrays.asList("dadKeys", l, "enrollmentKeys", pk));
						}
						break;
					case "personFirstName":
						putSql.append(SiteContextEnUS.SQL_setD);
	...
	public void putmergeSchoolDadResponse(SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
		response200PUTMergeSchoolDad(listSchoolDad, a -> {
			if(a.succeeded()) {
				SQLConnection sqlConnection = siteRequest.getSqlConnection();
				sqlConnection.commit(b -> {
					if(b.succeeded()) {
	...
	public void response200PUTMergeSchoolDad(SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		try {
			SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			eventHandler.handle(Future.succeededFuture(OperationResponse.completedWithJson(Buffer.buffer(JsonObject.mapFrom(apiRequest).encodePrettily()))));
														...
														if(e.succeeded()) {
															try {
																listPUTCopySchoolDad(apiRequest, listSchoolDad, f -> {
																	if(f.succeeded()) {
																		putcopySchoolDadResponse(listSchoolDad, g -> {
																			if(g.succeeded()) {
																				eventHandler.handle(Future.succeededFuture(g.result()));
		...
		CompositeFuture.all(futures).setHandler( a -> {
			if(a.succeeded()) {
				...
				if(listSchoolDad.next()) {
					...
					response200PUTCopySchoolDad(listSchoolDad, eventHandler);
				}
			} else {
	...
	public void putcopySchoolDadResponse(SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
		response200PUTCopySchoolDad(listSchoolDad, a -> {
			if(a.succeeded()) {
				SQLConnection sqlConnection = siteRequest.getSqlConnection();
				sqlConnection.commit(b -> {
					if(b.succeeded()) {
	...
	public void response200PUTCopySchoolDad(SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		try {
			SiteRequestEnUS siteRequest = listSchoolDad.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			eventHandler.handle(Future.succeededFuture(OperationResponse.completedWithJson(Buffer.buffer(JsonObject.mapFrom(apiRequest).encodePrettily()))));
	...
	public void aSearchSchoolDadQ(String classApiUriMethod, SearchList<SchoolDad> searchList, String entityVar, String valueIndexed, String varIndexed) {
	...
	public void aSearchSchoolDadFq(String classApiUriMethod, SearchList<SchoolDad> searchList, String entityVar, String valueIndexed, String varIndexed) {
		if(varIndexed == null)
			throw new RuntimeException(String.format("\"%s\" is not an indexed entity. ", entityVar));
		searchList.addFilterQuery(varIndexed + ":" + ClientUtils.escapeQueryChars(valueIndexed));
	}

	public void aSearchSchoolDadSort(String classApiUriMethod, SearchList<SchoolDad> searchList, String entityVar, String valueIndexed, String varIndexed) {
		if(varIndexed == null)
			throw new RuntimeException(String.format("\"%s\" is not an indexed entity. ", entityVar));
		searchList.addSort(varIndexed, ORDER.valueOf(valueIndexed));
	}

	public void aSearchSchoolDadRows(String classApiUriMethod, SearchList<SchoolDad> searchList, Integer valueRows) {
		searchList.setRows(valueRows);
	}

	public void aSearchSchoolDadStart(String classApiUriMethod, SearchList<SchoolDad> searchList, Integer valueStart) {
		searchList.setStart(valueStart);
	}

	public void aSearchSchoolDadVar(String classApiUriMethod, SearchList<SchoolDad> searchList, String var, String value) {
					...
					for(Object paramObject : paramObjects) {
						switch(paramName) {
							case "q":
								...
								aSearchSchoolDadQ(classApiUriMethod, searchList, entityVar, valueIndexed, varIndexed);
								break;
							case "fq":
								entityVar = StringUtils.trim(StringUtils.substringBefore((String)paramObject, ":"));
								valueIndexed = URLDecoder.decode(StringUtils.trim(StringUtils.substringAfter((String)paramObject, ":")), "UTF-8");
								varIndexed = SchoolDad.varIndexedSchoolDad(entityVar);
								aSearchSchoolDadFq(classApiUriMethod, searchList, entityVar, valueIndexed, varIndexed);
								break;
							case "sort":
								entityVar = StringUtils.trim(StringUtils.substringBefore((String)paramObject, " "));
								valueIndexed = StringUtils.trim(StringUtils.substringAfter((String)paramObject, " "));
								varIndexed = SchoolDad.varIndexedSchoolDad(entityVar);
								aSearchSchoolDadSort(classApiUriMethod, searchList, entityVar, valueIndexed, varIndexed);
								break;
							case "start":
								valueStart = (Integer)paramObject;
								aSearchSchoolDadStart(classApiUriMethod, searchList, valueStart);
								break;
							case "rows":
								valueRows = (Integer)paramObject;
								aSearchSchoolDadRows(classApiUriMethod, searchList, valueRows);
								break;
							case "var":
								entityVar = StringUtils.trim(StringUtils.substringBefore((String)paramObject, ":"));
								valueIndexed = URLDecoder.decode(StringUtils.trim(StringUtils.substringAfter((String)paramObject, ":")), "UTF-8");
								aSearchSchoolDadVar(classApiUriMethod, searchList, entityVar, valueIndexed);
								break;
						}
					}
				} catch(Exception e) {
