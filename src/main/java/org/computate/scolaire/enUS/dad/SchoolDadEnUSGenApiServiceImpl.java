...
public class SchoolDadEnUSGenApiServiceImpl implements SchoolDadEnUSGenApiService {
	...
	public void postSchoolDad(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		siteRequest.setRequestUri("/api/dad");
								...
								if(l2 != null) {
									futures.add(Future.future(a -> {
										tx.preparedQuery(SiteContextEnUS.SQL_addA)
												.execute(Tuple.of(l2, "dadKeys", pk, "enrollmentKeys")
												, b
										-> {
											if(b.succeeded())
												a.handle(Future.succeededFuture());
	...
	public void putimportSchoolDad(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolDad(siteContext, operationRequest, body);
		siteRequest.setRequestUri("/api/dad/import");
	...
	public void putmergeSchoolDad(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolDad(siteContext, operationRequest, body);
		siteRequest.setRequestUri("/api/dad/merge");
	...
	public void putcopySchoolDad(JsonObject body, OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolDad(siteContext, operationRequest, body);
		siteRequest.setRequestUri("/api/dad/copy");
	...
	public void listPUTCopySchoolDad(ApiRequest apiRequest, SearchList<SchoolDad> listSchoolDad, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		listSchoolDad.getList().forEach(o -> {
			...
			futures.add(
				putcopySchoolDadFuture(siteRequest2, JsonObject.mapFrom(o), a -> {
					if(a.succeeded()) {
					} else {
				...
				for(Integer i = 0; i < entityVars.size(); i++) {
					...
					case "enrollmentKeys":
						for(Long l : Optional.ofNullable(jsonObject.getJsonArray(entityVar)).orElse(new JsonArray()).stream().map(a -> Long.parseLong((String)a)).collect(Collectors.toList())) {
							futures.add(Future.future(a -> {
								tx.preparedQuery(SiteContextEnUS.SQL_addA)
										.execute(Tuple.of(l, "dadKeys", pk, "enrollmentKeys")
										, b
								-> {
									if(b.succeeded())
										a.handle(Future.succeededFuture());
										...
										try {
											aSearchSchoolDad(siteRequest, false, true, true, "/api/dad", "PATCH", d -> {
												if(d.succeeded()) {
													...
													if(listSchoolDad.getQueryResponse().getResults().getNumFound() > 1
															&& !CollectionUtils.containsAny(siteRequest.getUserResourceRoles(), roles2)
															&& !CollectionUtils.containsAny(siteRequest.getUserRealmRoles(), roles2)
															) {
														String message = String.format("roles required: " + String.join(", ", roles2));
														LOGGER.error(message);
														errorSchoolDad(siteRequest, eventHandler, Future.failedFuture(message));
													} else {
	...
	public void getSchoolDad(OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		...
		siteRequest.setRequestUri("/api/dad/{id}");
	...
	public void adminsearchSchoolDad(OperationRequest operationRequest, Handler<AsyncResult<OperationResponse>> eventHandler) {
		SiteRequestEnUS siteRequest = generateSiteRequestEnUSForSchoolDad(siteContext, operationRequest);
		siteRequest.setRequestUri("/api/admin/dad");
	...
	public void createSchoolDad(SiteRequestEnUS siteRequest, Handler<AsyncResult<SchoolDad>> eventHandler) {
		try {
			...
			tx.preparedQuery(SiteContextEnUS.SQL_create)
					...
					.execute(Tuple.of(SchoolDad.class.getCanonicalName(), userId, created.toOffsetDateTime())
					, createAsync
			-> {
	...
	public void aSearchSchoolDadFq(String uri, String apiMethod, SearchList<SchoolDad> searchList, String entityVar, String valueIndexed, String varIndexed) {
		...
		if(StringUtils.startsWith(valueIndexed, "[")) {
			...
			String fq1 = fqs[0].equals("*") ? fqs[0] : SchoolDad.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), fqs[0]);
			String fq2 = fqs[1].equals("*") ? fqs[1] : SchoolDad.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), fqs[1]);
			searchList.addFilterQuery(varIndexed + ":[" + fq1 + " TO " + fq2 + "]");
		} else {
			searchList.addFilterQuery(varIndexed + ":" + SchoolDad.staticSolrFqForClass(entityVar, searchList.getSiteRequest_(), valueIndexed));
		}
	}
	...
	public void aSearchSchoolDad(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, Handler<AsyncResult<SearchList<SchoolDad>>> eventHandler) {
		try {
			SearchList<SchoolDad> searchList = aSearchSchoolDadList(siteRequest, populate, store, modify, uri, apiMethod);
	...
	public SearchList<SchoolDad> aSearchSchoolDadList(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod) {
		OperationRequest operationRequest = siteRequest.getOperationRequest();
		...
		aSearchSchoolDad2(siteRequest, populate, store, modify, uri, apiMethod, searchList);
		searchList.initDeepForClass(siteRequest);
		return searchList;
	}
	public void aSearchSchoolDad2(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, Boolean modify, String uri, String apiMethod, SearchList<SchoolDad> searchList) {
	}
