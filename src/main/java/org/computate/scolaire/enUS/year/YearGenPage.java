...
public class YearGenPage extends YearGenPageGen<ClusterPage> {
	...
	@Override public void htmlScriptYearGenPage() {
		...
		if(
			...
			tl(2, "suggestSchoolYearEnrollmentKeys([{'name':'fq','value':'yearKey:' + pk}], $('#listSchoolYearEnrollmentKeys_Page'), pk, true); ");
		} else {
			tl(2, "suggestSchoolYearEnrollmentKeys([{'name':'fq','value':'yearKey:' + pk}], $('#listSchoolYearEnrollmentKeys_Page'), pk, false); ");
		}
		if(
				CollectionUtils.containsAny(siteRequest_.getUserResourceRoles(), ROLES)
				|| CollectionUtils.containsAny(siteRequest_.getUserRealmRoles(), ROLES)
		...
		try {
			...
			{ p.e("div").a("class", "w3-bar ").f();
				...
				p.e("input")
					...
					.a("autocomplete", "off")
					.a("oninput", "suggestSchoolYearObjectSuggest( [ { 'name': 'q', 'value': 'objectSuggest:' + $(this).val() }, { 'name': 'rows', 'value': '10' }, { 'name': 'fl', 'value': 'pk,pageUrlPk,yearCompleteName' } ], $('#suggestListSchoolYear", id, "'), ", p.getSiteRequest_().getRequestPk(), "); ")
