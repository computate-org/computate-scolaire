...
public abstract class MerePageGen<DEV> extends MereGenPage {
	...
	public void requetePatchMerePage() {
		RequetePatch requetePatch = Optional.ofNullable(requeteSite_).map(RequeteSiteFrFR::getRequetePatch_).orElse(null);
		MerePage original = (MerePage)Optional.ofNullable(requetePatch).map(RequetePatch::getOriginal).orElse(null);
		if(original != null) {
			super.requetePatchMereGenPage();
		}
	}
