...
public abstract class PerePageGen<DEV> extends PereGenPage {
	...
	public void requetePatchPerePage() {
		RequetePatch requetePatch = Optional.ofNullable(requeteSite_).map(RequeteSiteFrFR::getRequetePatch_).orElse(null);
		PerePage original = (PerePage)Optional.ofNullable(requetePatch).map(RequetePatch::getOriginal).orElse(null);
		if(original != null) {
			super.requetePatchPereGenPage();
		}
	}
